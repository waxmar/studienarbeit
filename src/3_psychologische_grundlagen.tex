...
Die \fachwort{Falken} repräsentieren dabei eine äußerst aggressive Strategie d.h. sie meiden einen Kampf nicht und ziehen sich erst zurück, wenn sie ernste körperliche Schäden erlitten haben.
\fachwort{Tauben} hingegen \ugspr{kämpfen} lediglich mit konventionellen Mitteln z.B. durch Drohungen oder ähnliche Aktionen.
Treffen zwei \fachwort{Tauben} aufeinander, so gibt es keinen Verletzten, da es keine tatsächliche Auseinandersetzung gibt.
Trifft eine \fachwort{Taube} auf einen \fachwort{Falken}, wird die \fachwort{Taube} schnell die Flucht ergreifen und der \fachwort{Falke} siegt.
...
Simuliert man dieses Modell wird man feststellen, dass die Anzahl der \fachwort{Tauben} grundsätzlich sehr hoch ist, da diese im Schnitt aus jeder zweiten Auseinandersetzung siegreich hervorgehen und kein Risiko tragen verletzt zu werden.
Wird das System jedoch dahingehend aus dem Gleichgewicht gebracht, dass es lukrativ ist, die Kampfstrategie des \fachwort{Falken} zu verfolgen, wird die Population der Tauben zurückgehen.
...
Die \fachwort{nonverbale Kommunikation} ist eine der wichtigsten Kommunikationskanäle.
Schätzungen einiger Experten zufolge macht die \fachwort{nonverbale Kommunikation} ca. 55\% des gesamten Informationsgehalts einer persönlich übermittelten Nachricht aus \citep[S. 4]{tu-dresden}.
Dementsprechend ist die Deutung der Informationen dieses Kanals entsprechend komplex, denn zur \fachwort{nonverbalen Kommunikation} zählen alle körperlichen Gesten, Bewegungen und Haltungen sowie auch akustische Eigenschaften, d.h. neben dem tatsächlich Gesagten, wird der Art wie etwas gesagt wird, ebenfalls eine Bedeutung zugewiesen \citep{hadnagy}.
...
\actor{Angreifer: }\says{Wären Sie dann so freundlich und sagen mir in welchem Zimmer sich Patient XY aufhält?}
