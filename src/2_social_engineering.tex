% Inormionen un inormiton gthering
% Risikofaktor Mensch
...
üblichen Vorgehensweisen des Hackens ab. Dabei wird in \Kapitel{sec:definition} zunächst eine
Definition gegeben.
In \Kapitel{sec:alternative_zu_technischen_methoden} wird die Einfachheit solcher Angriffe gegenüber
...
Der Angreifer versucht durch das Auslösen bestimmter Emotionen ein bestimmtes Verhalten zu triggern. Dabei handelt es sich um Automatismen, welche in \Kapitel{sec:automatismen} genauer beschrieben werden \citep{hadnagy}.
...
Diese Indizien sind in der Praxis sehr leicht zu fälschen. Mittels \fachwort{Spoofing} ist es mit besonders geringem Aufwand möglich, die auf dem Display angezeigte Rufnummer zu verfälschen.
...
Die Dauer eines solchen Angriffes kann dabei stark variieren.
...
Desweiteren beschrieben sie eine weitere Dimension des Identitätsbetrugs, welche im Allgemeinen unter einem verdeckten Ermittler, Spion oder \ugspr{Maulwurf} bekannt ist.
...
Die eben genannten Methoden machen sich ähnlich wie die unter Kapitel \nameref{sec:heimarbeitundhelpdesks} erwähnte Taktik dem im Allgemeinen als \ugspr{Helferinstinkt} bekannten Phänomen zu Nutze.
Dies geht stark mit dem Bedürfnis gemocht zu werden einher.
In \KapitelAndName{sec:psychologische_grundlagen} wird dieses Prinzip nochmals aufgegriffen und genauer erklärt.

Eine weitere Möglichkeit ist es einen Maulwurf in das Unternehmen zu schleusen.
...
Ebenso ist es denkbar, dass diese Rolle auch unwillentlich von einem Mitarbeiter eingenommen wird, z.B. durch Erpressung oder dass der Mitarbeiter gar unwissentlich in die Position eines \fachwort{Insiders} gerät \citep{Dhanjani2009}.
...
Warum gerade solche Angriffe eine besonders gute Erfolgsquote haben, ergibt sich vor allem aus der allgemeinen Auffassung des Begriffs \fachwort{Security}.
...
Der Angreifer macht sich dabei verschiedene menschliche Eigenschaften zu nutze (man könnte sie in diesem Zusammenhang auch als \ugspr{Sicherheitslücken} bezeichnen), wie sie später in Kapitel \vref{sec:psychologische_grundlagen} erläutert werden, zu Nutze.

Aus den im vorigen Kapitel aufgeführten Angriffstypen ist jedoch ein grundlegendes Schema herauszulesen.
...
Diese Hauptbestandteile \fachwort{Vertrauen}, \fachwort{Kommunikation} und \fachwort{Manipulation} im folgenden Kapitel genauer erläutert.

% Zusammenfassung: Was haben diese Methoden gemein?
% Vertrauen aufbauen
% Glaubwürdigkeit!
% Risikoanalyse nach psych. Grundlagen
